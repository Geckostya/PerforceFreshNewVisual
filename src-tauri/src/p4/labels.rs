...
use crate::models::{
    AppError, ConnectionInput, ErrorKind, Label, LabelInput, LabelSpec, LabelTagInput,
    LabelTagPreview, LabelTagResult,
};
...
pub fn update_label(input: &ConnectionInput, draft: &LabelInput) -> Result<LabelSpec, AppError> {
    let name = validate_label_name(&draft.name)?;
    if inspect_label(input, name)?.locked {
        return protected_label_error();
    }
    let spec = label_form(input, name, draft)?;
    submit_label_form(input, &spec)?;
    inspect_label(input, name)
...
pub fn preview_label_tag(
    ...
    Ok(LabelTagPreview {
        ...
        remove: tag.remove,
        scopes,
        protected: spec.locked,
        items: tag_items(&records),
        partial,
    })
}

pub fn apply_label_tag(
    input: &ConnectionInput,
    tag: &LabelTagInput,
    ...
    Ok(LabelTagResult {
        label: inspect_label(input, &spec.label.name)?,
        items: tag_items(&records),
        diagnostics,
        partial,
    })
...
fn validate_tag_input(tag: &LabelTagInput) -> Result<Vec<String>, AppError> {
    if tag.paths.is_empty() || tag.paths.len() > 200 {
        return Err(AppError::new(
            ErrorKind::CommandFailed,
            "Выберите от одного до 200 depot paths.",
        ));
    }
    let paths = tag
        .paths
...
fn protected_label_error<T>() -> Result<T, AppError> {
    Err(AppError::new(
        ErrorKind::Permission,
        "Label защищён или заблокирован сервером.",
    )
    .with_hint("Снимите защиту в Perforce или обратитесь к владельцу label."))
}

fn label_form(input: &ConnectionInput, name: &str, draft: &LabelInput) -> Result<String, AppError> {
    if draft.description.len() > 10_000
        || draft.view.len() > 200
        || draft
            .view
            .iter()
            .any(|line| line.trim().is_empty() || line.contains(['\r', '\n', '\0']))
    {
        return Err(AppError::new(
            ErrorKind::CommandFailed,
            "Некорректные поля label.",
        ));
    }
    validate_depot_paths(
        &draft
            .view
            .iter()
...
fn submit_label_form(input: &ConnectionInput, spec: &str) -> Result<(), AppError> {
    ...
    if output.status.success() {
        ...
        Err(super::command_error(&output))
    }
}

fn parse_label_spec(spec: &str) -> Result<LabelSpec, AppError> {
    let lines = spec.lines().collect::<Vec<_>>();
    let name = form_value(&lines, "Label").ok_or_else(|| {
        AppError::new(ErrorKind::InvalidOutput, "В label form отсутствует Label.")
    })?;
    let owner = form_value(&lines, "Owner");
    let update = form_value(&lines, "Update");
    let description = form_multiline(&lines, "Description");
    let view = form_multiline(&lines, "View")
        .lines()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(str::to_owned)
        .collect();
    let locked = form_value(&lines, "Options").is_some_and(|value| {
        value
            .split_whitespace()
            .any(|option| option.eq_ignore_ascii_case("locked"))
    });
    Ok(LabelSpec {
        label: Label {
            name,
...
fn form_value(lines: &[&str], field: &str) -> Option<String> {
    lines
        .iter()
...
fn form_multiline(lines: &[&str], field: &str) -> String {
    let Some(start) = lines
        .iter()
        .position(|line| line.starts_with(&format!("{field}:")))
    else {
        return String::new();
    };
    lines[start + 1..]
...
fn replace_single(lines: &mut [String], field: &str, value: &str) -> Result<(), AppError> {
    let line = lines
        .iter_mut()
        ...
        .ok_or_else(|| {
            AppError::new(
                ...
                format!("В label form отсутствует {field}."),
            )
        })?;
    *line = format!("{field}:\t{value}");
    Ok(())
}
fn replace_multiline(lines: &mut Vec<String>, field: &str, value: &str) -> Result<(), AppError> {
    let start = lines
        .iter()
        .position(|line| line.starts_with(&format!("{field}:")))
        .ok_or_else(|| {
    ...
    let end = lines[start + 1..]
        ...
        .unwrap_or(lines.len());
    let replacement = std::iter::once(format!("{field}:"))
        .chain(
            value
                .lines()
...
fn tag_items(records: &[Map<String, Value>]) -> Vec<String> {
    records
        .iter()
        .filter(|record| !is_message_record(record))
        .filter_map(|record| optional_field(record, &["depotFile", "clientFile", "path", "data"]))
        .collect()
}
