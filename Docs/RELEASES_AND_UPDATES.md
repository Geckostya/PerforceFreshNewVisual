...
P4FNV ships as a Windows x64 portable ZIP. Users extract it to a writable directory and start `p4fnv.exe`; P4FNV itself needs no installer or administrator access. It uses the system WebView2 Runtime, included with Windows 11 and normally serviced on Windows 10; minimal images must install Microsoft's Evergreen runtime first. A future installer requires demonstrated need and is not a release prerequisite.
...
The normal trigger is a matching tag push. If GitHub does not deliver that event, dispatch `Portable release` with the immutable release tag; it checks out and builds that tag, then `publish-release.ps1 -ResumeDraft` performs the same verification and publication.
...
2. **Reproducible GitHub publishing — hosted-run verified.** The `v0.1.3` workflow passed the full gate, signed the archive, created the six-asset release, and attached SLSA provenance for commit `202377211b86756761e8d365d356a866a510fbf6`. The downloaded assets passed `verify-release-assets.ps1` asset-set, signature, manifest, hash, repository, and tag checks.

3. **Release trust chain — configured and release-verified.** Two protected backups and the private/public GitHub Secrets exist. The hosted `p4fnv.exe` contains the public key; a deliberately incorrect key rejected the release, while the configured key accepted its Ed25519 signatures, SHA-256 hash, manifest, and SLSA provenance. `v0.1.3` was published only after this verification.
...
5. **Portable replacement — public end-to-end verified.** A native-agent smoke upgraded portable `0.1.2` through the public GitHub `latest.json`, relaunched `0.1.3`, matched every managed-file hash, left no transaction state, preserved unmanaged files, and kept app-config settings and workspace cache byte-for-byte unchanged. Automated tests cover locked and unwritable targets, cancellation, invalid signatures/hashes, rollback, and interrupted recovery.

6. **First public release — verified.** `publish-release.ps1` published `v0.1.3` after the GitHub-built ZIP launched in a clean Windows Sandbox profile. The immediately previous `0.1.2` then discovered the public release, downloaded it, installed it, and relaunched `0.1.3` successfully.
