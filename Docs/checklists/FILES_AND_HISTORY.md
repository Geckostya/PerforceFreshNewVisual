...
- [ ] Allow explicit reconcile preview/apply for any mapped folder selected inside the current workspace, with folder-boundary validation and stale protection.
- [ ] Discover unopened local changes in configured roots without mutating the workspace; expose coverage, ignored files, partial scans, and a virtual My Changes group.
