...
Background workspace discovery has a separate bounded backend cache because it can run without the Files screen. Rust stores up to eight workspace scopes in a gzip-compressed, atomically replaced file under the application config directory. Each scope keeps directory-only fingerprints (entry shape, direct-file count, newest direct-file modification time, and a small digest) plus the last candidates; file contents and server state are never cached. A normal cycle snapshots local metadata, revalidates only changed directory scopes in batches, and resumes its target cursor after the time budget. A Configure/Refresh request and a periodic fifteen-minute safety pass force a full server-authoritative scan. A missing, corrupt, stale, or scope-mismatched cache is treated as a miss, never as authority.
