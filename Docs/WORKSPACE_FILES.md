...
Background workspace discovery has a separate bounded backend cache because it can run without the Files screen. Rust stores up to eight workspace scopes in a gzip-compressed, atomically replaced file under the application config directory. Each scope keeps directory-only fingerprints (entry shape, direct-file count, newest direct-file modification time, and a small digest), validation timestamps, and the last candidates; file contents and server state are never cached. An existing matching cache restores candidates and completes immediately on application reopen, then validates on the next normal cycle; Configure and explicit Refresh still start immediately. Bounded local validation advances one directory at a time, reports the current folder and checked/known folder count, and yields after the time budget. Only changed directory scopes are then revalidated through Perforce in batches; a single changed folder must remain a narrow scope and added-file checks use those exact local folders. An unchanged cache completes without a server command and is written once after the whole validation, not once per root. A timed-out or failed background command produces a terminal Partial result, retains its completed-root and target cursor, and waits for the next normal cycle instead of retrying in a tight loop. The Changes screen's shared Refresh action resumes that cursor immediately or starts the same cache-aware validation after a completed cycle; periodic validation never expands an unchanged cache into an automatic full-root scan. A missing, corrupt, or scope-mismatched cache is treated as a miss, never as authority; cached candidates remain presentation-only until revalidated.
