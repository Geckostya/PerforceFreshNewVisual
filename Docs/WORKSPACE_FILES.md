...
- The top `Local files` / `Depot files` switch changes the source within one screen instead of creating a separate navigation item.
...
- Ignored files and directories receive muted orange text according to `p4 ignores -i`, but no separate orange background. A branch receives the same text accent when all displayed files inside it are ignored. Depot files use a blue accent; local-only entries in depot mode remain visible, gray, and disabled.
...
- Selecting a file-history row defines the exact `file#revision` for the visible `Update selected`; selecting a submitted changelist for a folder defines `folder/...@change`. There is no separate Target Revision field.
...
The force operation succeeds only when the entire snapshot is applied successfully and the final `p4 sync -n` is empty. A completed result closes the dialog, while any failure preserves the entire explicitly selected set and its decisions for a safe idempotent retry; the have list alone is not trustworthy.
