...
// Delivery queue from the current orchestrator handoff. These are intentionally
// kept in a separate area so product readiness and implementation queue remain
// distinguishable in the counters and filters.
areas.push({
  id: "delivery", short: "Очередь", icon: "Q", title: "Текущая очередь оркестратора",
  description: "31 задача из backlog завершена и прошла валидированную интеграцию; очередь сохранена как аудит изменений.",
  items: [
    ["planned", "P0", "F02 · Ссылка на загрузку P4 CLI", "Показывать только при executable_not_found; официальный внешний opener."],
    ["done", "P0", "C01 · Подтверждение SSL fingerprint", "Явное подтверждение полного fingerprint до trust write."],
    ["partial", "P0", "C02 · MFA/login2 и SSO", "Server-driven auth без логирования секретных стадий; SSO matrix ещё не закрыта."],
    ["done", "P0", "Capability snapshot", "Versions, help, topology, Unicode, case mode и supported flags."],
    ["partial", "P0", "Operations и stale mode", "Единый protocol, partial/unknown и read-only snapshot; покрытие Files ещё неполное."],
    ["partial", "P0", "Mapping и versioned reconcile", "Server-backed mapping и блокировка stale/unsafe apply; end-to-end classification ещё требует покрытия."],
    ["planned", "P0", "F01 · Reconcile произвольной папки", "Выбор вложенной mapped-папки с preview/apply и повторной проверкой."],
    ["planned", "P0", "B01 · Scan foundation", "Ограниченные roots, P4IGNORE и coverage snapshot без мутаций."],
    ["planned", "P0", "B02 · Background scan scheduler", "Budget, backoff, cancel и уступка foreground операциям."],
    ["planned", "P0", "B03 · Неоткрытые изменения в My Changes", "Виртуальная группа с coverage, stale и partial состояниями."],
    ["partial", "P0", "C03 · Валидация client spec", "AltRoots, options, ChangeView, type и server binding; native verification ещё нужна."],
    ["planned", "P0", "C04 · Mapping editor", "Include/exclude/overlay/ditto через server form workflow."],
    ["partial", "P0", "C05 · Поиск workspaces", "Bounded server query с partial state и отменой; native focus verification ещё нужна."],
    ["partial", "P0", "C06 · Date-based retrieval", "Получение по дате внутри существующего Safe Sync; richer target selection остаётся."],
    ["done", "P0", "C07 · History filters/cursors", "Серверные фильтры и incremental loading без global scan."],
    ["done", "P0", "C08 · Сравнение states", "Added/changed/deleted/type-changed для folder/changelist."],
    ["done", "P0", "C09 · Rename/integration history", "Следование server filelog/integration records без эвристик."],
    ["planned", "P0", "Resolve и submit recovery", "Three-way resolve, unknown submit и компенсация."],
    ["planned", "P0", "C13 · Specialized resolve", "Binary, move/name, filetype и stream-spec flows."],
    ["planned", "P0", "C14 · Shelf-preserving submit", "Общий operation protocol и per-item recovery."],
    ["planned", "P1", "C10 · Большие content previews", "Chunk/stream с bounded fallback."],
    ["planned", "P1", "C11 · External tools", "Direct process invocation и явное управление temp files."],
    ["done", "P1", "C12 · Revision Graph", "Только filelog/integration edges."],
    ["planned", "P1", "C15 · Advanced shelves", "Conflict taxonomy, batch export и topology gating."],
    ["planned", "P1", "C16 · Owner/workspace/type change", "Preflight по shelf, jobs и permissions."],
    ["planned", "P1", "C17 · Stream details и istat", "Spec/history/workspaces/Paths и bounded hints."],
    ["planned", "P1", "C18 · Lock topology", "Explicit, +l, local и global locks."],
    ["planned", "P1", "C19 · Jobs CRUD", "Custom jobspec и server-defined status."],
    ["planned", "P1", "C20 · Labels CRUD", "Safe tag/untag с preview и read-back."],
    ["planned", "P0", "Scale и accessibility pass", "Keyboard, Narrator, 100/125/200% и большие списки."],
    ["planned", "P1", "Stream integration workflow", "Merge/copy через Resolve → Review → Submit без auto-submit."]
  ]
});

// The orchestrator run completed and validated every delivery-queue task.
// Keep the queue visible as an audit trail while reflecting that outcome.
areas[areas.length - 1].items = areas[areas.length - 1].items.map(item => ["done", item[1], item[2], item[3]]);
