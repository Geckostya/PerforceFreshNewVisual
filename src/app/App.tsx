...
function AppContent() {
  ...
  const [changeAction, setChangeAction] = useState<{ id: number; change: string; openSubmit: boolean }>();
  const [integrationResolve, setIntegrationResolve] = useState<{ id: number; change: string; paths: string[] }>();
        ...
        <main className="workspace-main" data-focus-pane tabIndex={-1}>
          <Suspense fallback={<div className="compact-empty" role="status">{t("loadingFiles")}</div>}>
            <div className="workspace-view-host" hidden={view !== "workspace" || filesSource !== "local"}>
              <WorkspaceView connection={session.connection} info={session.info} initialScope={workspaceScope} initialResolveRequest={integrationResolve} sourceControl={<FilesSourceControl source="local" setSource={setFilesSource} />} onNavigateDepot={(scope) => { setDepotScope(scope); setFilesSource("depot"); }} />
            </div>
            <div className="workspace-view-host" hidden={view !== "workspace" || filesSource !== "depot"}><DepotView connection={session.connection} initialScope={depotScope} sourceControl={<FilesSourceControl source="depot" setSource={setFilesSource} />} onNavigateLocal={(scope) => { setWorkspaceScope(scope); setFilesSource("local"); }} /></div>
            <div className="workspace-view-host" hidden={view !== "changes"}><ChangesView connection={session.connection} info={session.info} onFileCountChange={handleFileCount} initialChange={changeTarget} initialAction={changeAction} /></div>
            <div className="workspace-view-host" hidden={view !== "streams"}><StreamsView connection={session.connection} currentStream={session.info.clientStream} capabilities={session.info.capabilities} onSwitched={(info) => setSession((current) => current ? { ...current, info } : current)} onResolveIntegration={(change, paths) => { setIntegrationResolve({ id: Date.now(), change, paths }); setFilesSource("local"); setView("workspace"); }} onReviewIntegration={(change, openSubmit) => { setChangeTarget(change); setChangeAction({ id: Date.now(), change, openSubmit }); setView("changes"); }} /></div>
            <div className="workspace-view-host" hidden={view !== "history"}><HistoryView connection={session.connection} info={session.info} /></div>
            <div className="workspace-view-host" hidden={view !== "jobs"}><JobsView connection={session.connection} initialSearch={jobSearch} /></div>
            <div className="workspace-view-host" hidden={view !== "labels"}><LabelsView connection={session.connection} initialSearch={labelSearch} /></div>
            <div className="workspace-view-host" hidden={view !== "shelves"}><ShelvesView connection={session.connection} info={session.info} /></div>
          </Suspense>
        </main>
      </div>
