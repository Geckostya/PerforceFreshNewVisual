...
export function WorkspaceView({ connection, info, initialScope, initialResolveRequest, sourceControl, onNavigateDepot }: { connection: ConnectionInput; info: P4Info; initialScope?: string; initialResolveRequest?: { id: number; change: string; paths: string[] }; sourceControl?: ReactNode; onNavigateDepot?: (scope: string) => void }) {
  ...
  useEffect(() => { void load(); }, [connection.port, connection.user, connection.client, info.clientRoot, info.clientStream, cacheVersion]);
  ...
  async function showReconcilePreview(nextScope = scope, label = nextScope) {
    setBusy(true);
    setError(undefined);
    setNotice("");
    setReconcilePreviewScope({ depot: nextScope, label });
    try {
      await startObservedOperation("reconcile_preview", () => startReconcilePreview(connection, nextScope), (event) => {
  ...
  async function applyReconcile() {
    ...
    const previousScope = reconcilePreviewScope;
    setReconcileCandidates(undefined);
    setBusy(true);
    setError(undefined);
    setNotice("");
    try {
      await startObservedOperation("reconcile", () => startReconcile(connection, change, previousScope.depot, reconcileSelected), (event) => {
        setReconcileOperation(isOperationTerminal(event.kind) ? undefined : event);
        if (!isOperationTerminal(event.kind)) return;
        void (async () => {
          try {
            ...
            if (event.kind === "completed") {
              setReconcilePreviewScope(undefined);
              setNotice(t("reconcileSucceeded"));
            } else if (event.kind === "cancelled") {
              setReconcilePreviewScope(undefined);
              setNotice(t("reconcileCancelled"));
            } else {
  ...
  >
    ...
    {reconcileCandidates && reconcilePreviewScope && <ActionDialog wide className="reconcile-preview-dialog" title={t("reconcilePreviewTitle")} confirmLabel={t("applyReconcile")} busy={busy} confirmDisabled={!reconcileSelected.length || reconcileStale} onClose={() => { setReconcileCandidates(undefined); setReconcilePreviewScope(undefined); }} onConfirm={() => void applyReconcile()}><p>{reconcileStale ? t("reconcileStaleBody") : t("reconcilePreviewBody")}</p><p title={`${reconcilePreviewScope.label} → ${reconcilePreviewScope.depot}`}><strong>{t("reconcileScope")}:</strong> {reconcilePreviewScope.label}{reconcilePreviewScope.label !== reconcilePreviewScope.depot ? ` → ${reconcilePreviewScope.depot}` : ""}</p>{reconcileStale && <button className="secondary-button" type="button" onClick={() => void showReconcilePreview(reconcilePreviewScope.depot, reconcilePreviewScope.label)}>{t("refreshReconcilePreview")}</button>}<div className="resource-detail-list reconcile-preview-list">{reconcileCandidates.length ? reconcileGroups.map((group) => <section key={group.action} className="reconcile-group"><h3>{t(`reconcileGroup_${group.action}` as never)} · {group.items.length}</h3>{group.items.map((item) => { const disabled = item.ignored || item.unsafeItem || item.action === "unsafe"; const reasons = item.reasons.map((reason) => t(`reconcileReason_${reason}` as never)).join(" · "); return <SelectableRow aria-disabled={disabled} selected={reconcileSelected.some((selectedItem) => selectedItem.stableId === item.stableId)} className="resource-entry preview-select" key={item.stableId} onClick={() => setReconcileSelected((current) => toggleReconcileSelection(current, item))}><ItemRowCopy primary={item.depotPath} secondary={<>{t(`reconcileGroup_${item.action}` as never)}{item.localPath ? ` · ${item.localPath}` : ""}{reasons ? ` · ${reasons}` : ""}</>} /></SelectableRow>; })}</section>) : <CompactEmpty text={t("noReconcileChanges")} />}</div></ActionDialog>}
