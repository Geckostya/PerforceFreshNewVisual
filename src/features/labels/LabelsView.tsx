...
import type { AppError, ConnectionInput, DepotFile, Label, LabelInput, LabelSpec, LabelTagPreview, LabelTagResult, SyncPreview } from "../../shared/models";
...
function tagPaths(value: string): string[] {
...
export function LabelsView({ connection, initialSearch }: { connection: ConnectionInput; initialSearch?: string }) {
  ...
  const [tagResult, setTagResult] = useState<LabelTagResult>();
  ...
  async function applyDialog() {
    ...
    try {
      ...
      else if ((dialog.kind === "tag" || dialog.kind === "untag") && selectedLabel) { const tag = { label: selectedLabel.name, paths: tagPaths(dialog.paths), remove: dialog.kind === "untag" }; if (!dialog.preview) { setDialog({ ...dialog, preview: await previewLabelTag(connection, tag) }); } else { const result = await applyLabelTag(connection, tag); setDialog(undefined); await inspectLabel(result.label.label, result.label); setTagResult(result); setNotice(result.partial ? t("labelTagPartial") : t("labelTagApplied")); } }
    } catch (reason) { setError(normalizeAppError(reason)); }
    finally { setDetailBusy(false); }
  }
  ...
  >
    ...
    <div className="resource-workbench">
      ...
      <aside className="resource-inspector">
        ...
        {!selectedLabel ? <EmptyState title={t("labelsTitle")} body={t("labelsBody")} /> : <div className="inspector-content">
          ...
          <div className="dialog-actions"><button className="secondary-button" type="button" onClick={() => void openEdit("edit")} disabled={detailBusy}>{t("editLabel")}</button><button className="secondary-button" type="button" onClick={() => setDialog({ kind: "tag", paths: "" })} disabled={detailBusy}>{t("tagLabel")}</button><button className="secondary-button" type="button" onClick={() => setDialog({ kind: "untag", paths: labelFiles.map((file) => file.depotPath).join("\n") })} disabled={detailBusy || !labelFiles.length}>{t("untagLabel")}</button><button className="danger-button" type="button" onClick={() => setDialog({ kind: "delete" })} disabled={detailBusy}>{t("deleteLabel")}</button></div>
          ...
          {tagResult && <div className={`notice-banner${tagResult.partial ? " warning" : ""}`}><strong>{t("labelTagResults")}</strong><span>{tagResult.items.length} {t("filesCount")}</span>{tagResult.items.length > 0 && <div className="file-selection-summary">{tagResult.items.map((item) => <span key={item}>{item}</span>)}</div>}{tagResult.diagnostics.map((diagnostic) => <span key={diagnostic}>{diagnostic}</span>)}</div>}
        </div>}
      </aside>
    </div>
    ...
    {(dialog?.kind === "tag" || dialog?.kind === "untag") && selectedLabel && <ActionDialog title={dialog.kind === "tag" ? t("tagLabel") : t("untagLabel")} confirmLabel={dialog.preview ? (dialog.kind === "tag" ? t("tagLabel") : t("untagLabel")) : t("preview")} busy={detailBusy} confirmDisabled={dialog.preview ? Boolean(dialog.preview.protected || dialog.preview.partial) : tagPaths(dialog.paths).length === 0} onClose={() => setDialog(undefined)} onConfirm={() => void applyDialog()}><p>{dialog.preview ? t("labelTagPreviewReady") : t("labelTagPreviewPrompt")}</p><label className="field"><span className="field-label">{t("labelTagPaths")}</span><textarea autoFocus disabled={Boolean(dialog.preview)} value={dialog.paths} onChange={(event) => setDialog({ ...dialog, paths: event.target.value, preview: undefined })} /></label>{dialog.preview && <><dl className="dialog-facts"><dt>{t("labelScope")}</dt><dd>{dialog.preview.scopes.length}</dd><dt>{t("labelProtected")}</dt><dd>{dialog.preview.protected ? t("labelProtected") : t("no")}</dd></dl><div className="file-selection-summary">{dialog.preview.scopes.map((scope) => <span key={scope}>{scope}</span>)}</div></>}</ActionDialog>}
