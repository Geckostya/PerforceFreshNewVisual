...
export function UndoDialog({ connection, sourceChange, onClose, onComplete }: {
  ...
  return <div className="dialog-layer"><section className="action-dialog wide" role="dialog" aria-modal="true" aria-labelledby="undo-dialog-title">
    <div className="dialog-heading"><div><h2 id="undo-dialog-title">{t("undoTitle")}</h2><p>{t("undoBody")}</p></div><button type="button" onClick={onClose} disabled={busy} aria-label={t("close")}><X className="ui-icon" aria-hidden="true" /></button></div>
    {error && <div className="error-banner" role="alert"><strong>{error.message}</strong>{error.hints.map((hint) => <span key={hint}>{hint}</span>)}</div>}
    <div className="dialog-body"><dl className="dialog-facts"><dt>{t("undoSource")}</dt><dd className="changelist-number">CL {sourceChange}</dd></dl><label className="field"><span className="field-label">{t("undoTarget")}</span><input value={targetChange} onChange={(event) => { setTargetChange(event.target.value); setPreviewed(false); }} placeholder="default or 12345" /></label>{previewed && <><p>{items.length ? `${items.length} ${t("filesCount")}` : t("undoEmpty")}</p>{items.map((item) => <div className="preview-row" key={`${item.depotPath}-${item.action}`}><span>{item.action}</span><span>{item.depotPath}</span><span>{item.localPath || ""}</span></div>)}</>}</div>
