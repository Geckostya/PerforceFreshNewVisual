...
export function buildRevisionGraph(revisions: FileRevision[], historyMayBePartial = false): RevisionGraphModel {
  ...
  for (const revision of visibleRevisions) {
    for (const integration of revision.integrations) {
      ...
      const range = [integration.sourceStartRevision, integration.sourceEndRevision].filter(Boolean).join("–");
      edges.push({
        source: `${integration.sourcePath}${range ? `#${range}` : ""}`,
