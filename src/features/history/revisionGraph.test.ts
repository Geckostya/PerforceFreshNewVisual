...
describe("buildRevisionGraph", () => {
  it("uses only server integration records as edges", () => {
    const graph = buildRevisionGraph([{ revision: "7", change: "12", action: "edit", user: "alex", integrations: [{ how: "merge", sourcePath: "//Acme/dev/a.txt", sourceStartRevision: "3", sourceEndRevision: "6" }], labels: [] }]);
  ...
  it("does not invent an edge when filelog omits its source path", () => {
    const graph = buildRevisionGraph([{ revision: "7", change: "12", action: "edit", user: "alex", integrations: [{ how: "merge" }], labels: [] }]);
    expect(graph.edges).toEqual([]);
    expect(graph.partial).toBe(true);
  });

  it("bounds graph records and reports a partial graph", () => {
    const revision = (number: number) => ({ revision: String(number), change: String(number), action: "edit", user: "alex", integrations: Array.from({ length: 3 }, () => ({ how: "merge", sourcePath: "//Acme/dev/a.txt" })), labels: [] });
