...
function DetectionStatus({ detection, onRetry, t }: { detection: DetectionState; onRetry: () => void; t: Translate }) {
  if (detection.phase === "loading") {
    return <div className="tool-status" role="status"><span className="status-symbol" aria-hidden="true">···</span><div className="tool-status-copy"><strong>{t("toolSearching")}</strong><span>{t("toolSearchingBody")}</span></div></div>;
  }
  if (detection.phase === "error") {
    return <div className="tool-status error" role="alert"><span className="status-symbol" aria-hidden="true">!</span><div className="tool-status-copy"><strong>{errorText(detection.error.kind, t).message}</strong><span>{errorText(detection.error.kind, t).hint}</span></div><button className="link-button" type="button" onClick={onRetry}>{t("findAgain")}</button></div>;
  }
  return <div className="tool-status success" role="status"><span className="status-symbol" aria-hidden="true">✓</span><div className="tool-status-copy"><strong>{t("toolReady")}</strong><span title={`${detection.value.path} · ${detection.value.version}`}>{detection.value.path} · {detection.value.version}</span></div><button className="link-button" type="button" onClick={onRetry}>{t("findAgain")}</button></div>;
}
